...
const {setupOrder, setupV3Order} = require("./utils/order");
...
describe("V3Fill", function() {
    ...
    context("----Settlement With Order----", function() {
        ...
        before(async function() {
            ...
            props.quote = res;
            const quote = res;
            console.log("FEE TOKEN ETH RATE", quote.feeTokenToGasTokenRate);

            fees = quote.fees;
            const feesPerRound = quote.perRoundFees;
            const orderDetails = {
                feeToken: fees.token,
                feeTokenETHPrice: inUnits((1/quote.feeTokenToGasTokenRate).toFixed(12), 18),
                dexibleFee: bn(feesPerRound.provider),
                gasFee: bn(feesPerRound.gas),
                trader: TRADER,
                input: {
                    token: TOKEN_IN,
                    amount: bn(quote.perRoundInputAmounts.forContract),
                },
                output: {
                    token: TOKEN_OUT,
                    amount: bn(quote.perRoundOutputAmounts.forContract)
                }
            }
            ...
            //console.log("RES", res);
            console.log("Original quote output", quote.buyAmountAfterFees.toString());

            console.log("Expected gas", quote.estimatedGasUsage);
            maxGas = GAS_PRICE;
            whale = await ethers.provider.getSigner(TRADER);
            console.log("Whale ETH balance", await ethers.provider.getBalance(TRADER));
            const minBuyAmount = bn(quote.buyAmountAfterFees); //minGrossOutput);
            console.log("Min buy", minBuyAmount.toString());

            order = setupV3Order(orderDetails);
           
            console.log("Order", order);
            let abi = ethers.utils.defaultAbiCoder;
            console.log("Allowance target", quote.source.to);
            encodedCallData = abi.encode(["address","address", "bytes"], [quote.source.allowanceTarget,quote.source.to,quote.source.data]);
            console.log("Encoded data", encodedCallData);
        ...
        it("Should swap with valid order", async function() {
            ...
            console.log("Checking owner", props.ownerAddress, "fee token balance", order.fees.feeToken);

            let feeErc20 = new ethers.Contract(order.fees.feeToken, erc20ABI, ethers.provider);
            let beforeBal = await feeErc20.balanceOf(props.ownerAddress);
            console.log("Bal", beforeBal.toString());

            console.log("Calling settlement", props.settlementContract.address, "with relay", props.relay.address);

            let txn = await props.settlementContract.connect(props.relay).v3Fill(order, ethers.constants.AddressZero, encodedCallData, {
                gasPrice: gp,
                gasLimit: bn(2000000)
            });
            console.log("Finished");
            await expect(txn).to.emit(props.settlementContract, "SwapV3Success");
